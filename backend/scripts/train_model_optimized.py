...
async def main():  # noqa: C901
    ...
    async def generate_league_predictions(
        league_id, league_matches, team_stats_cache, league_avgs, models_bundle=None
    ):
        try:
            # fetch upcoming
            ...
            for match in upcoming:
                ...
                try:
                    ...
                    winner_model = (
                        models_bundle.get("winner") if models_bundle else None
                    )
    ...
    for league_id, league_matches in matches_by_league.items():
        ...
        logger.info(
            f"🏟️ Processing League: {league_id} ({len(league_matches)} matches)"
        )
        ...
        predicted_batch = await generate_league_predictions(
            league_id, league_matches, team_stats_cache, league_avgs, models_bundle
        )
        if predicted_batch:
