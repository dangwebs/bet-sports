...
class MatchEnrichmentService:
    ...
    def merge_matches(
        self, primary_matches: List[Match], secondary_matches: List[Match]
    ) -> List[Match]:
        """
    ...
    def _enrich_match(self, base: Match, source: Match):
        ...
        if base.home_goals is None:
            base.home_goals = source.home_goals
        if base.away_goals is None:
            base.away_goals = source.away_goals

        # Corners (High value for picks)
        if base.home_corners is None:
            base.home_corners = source.home_corners
        if base.away_corners is None:
            base.away_corners = source.away_corners

        # Yellow Cards
        if base.home_yellow_cards is None:
            base.home_yellow_cards = source.home_yellow_cards
        if base.away_yellow_cards is None:
            base.away_yellow_cards = source.away_yellow_cards

        # Red Cards
        if base.home_red_cards is None:
            base.home_red_cards = source.home_red_cards
        if base.away_red_cards is None:
            base.away_red_cards = source.away_red_cards

        # Betting Odds (Crucial for EV/Training)
        if base.home_odds is None:
            base.home_odds = source.home_odds
        if base.draw_odds is None:
            base.draw_odds = source.draw_odds
        if base.away_odds is None:
            base.away_odds = source.away_odds

        # Shots (Advanced stats for training)
        if hasattr(base, "home_shots") and hasattr(source, "home_shots"):
            if base.home_shots is None:
                base.home_shots = source.home_shots
        if hasattr(base, "away_shots") and hasattr(source, "away_shots"):
            if base.away_shots is None:
                base.away_shots = source.away_shots

        # Update timestamp
        if source.data_fetched_at:
            if (
                not base.data_fetched_at
                or source.data_fetched_at > base.data_fetched_at
            ):
                base.data_fetched_at = source.data_fetched_at

    def find_match_overlap(
        self, target: Match, candidates: List[Match]
    ) -> Optional[Match]:
        """
