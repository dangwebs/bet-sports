...
class PickResolutionService:
    ...
    def resolve_pick(self, pick: SuggestedPick, match: Match) -> Tuple[str, float]:
        ...
        try:
            ...
            if (
                "Más de" in pick.market_label
                or "Menos de" in pick.market_label
                or "Over" in pick.market_label
                or "Under" in pick.market_label
            ):
            ...
            if market_type_str in ["winner", "draw", "result_1x2"]:
                ...
                predicted = "X"
                if "(1)" in pick.market_label:
                    predicted = "1"
                elif "(2)" in pick.market_label:
                    predicted = "2"
                was_correct = actual == predicted

            elif "over" in market_type_str:
                total = 0
                if "corners" in market_type_str:
                    total = (match.home_corners or 0) + (match.away_corners or 0)
                elif "cards" in market_type_str:
                    total = (match.home_yellow_cards or 0) + (
                        match.away_yellow_cards or 0
                    )
                elif "goals" in market_type_str:
