...
class ParleyConfig:
    min_probability: float = (
        0.50  # Lowered to allow Value Bets (e.g. Odds 2.10 with 55% prob)
    )
    min_picks: int = 3
    max_picks: int = 5
    count: int = 3
...
class ParleyService:
    ...
    def generate_parleys(
        self, predictions: List[MatchPrediction], config: ParleyConfig
    ) -> List[Parley]:
        """
        Generates a list of suggested parleys based on predictions and configuration.
        """
        # 1. Filter eligible picks from predictions
        eligible_picks = self._filter_eligible_picks(
            predictions, config.min_probability
        )
    ...
    def _filter_eligible_picks(
        self, predictions: List[MatchPrediction], min_probability: float
    ) -> List[SuggestedPick]:
        """
        ...
        for pred in predictions:
            ...
            top_ml_picks = [
                ...
                if (
                    "ML Confianza Alta" in (p.reasoning or "")
                    and p.probability >= min_probability
                )
            ]
    ...
    def _combine_picks(
        self, picks: List[SuggestedPick], config: ParleyConfig
    ) -> List[Parley]:
        """
