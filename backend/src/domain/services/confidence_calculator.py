...
class ConfidenceCalculator:
    ...
    def calculate_pick_confidence(
        ...
        odds_alignment = self._check_odds_alignment(
            market_type, probability, prediction
        )
        ...
        weighted_score = (
            data_quality * 0.30 + odds_alignment * 0.30 + stat_strength * 0.40
        )
    ...
    def _assess_data_quality(
        self, home_stats: Optional[TeamStatistics], away_stats: Optional[TeamStatistics]
    ) -> float:
        """
    ...
    def _check_odds_alignment(
        self, market_type: MarketType, model_prob: float, prediction: Prediction
    ) -> float:
        """
