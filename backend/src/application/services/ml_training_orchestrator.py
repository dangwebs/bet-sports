...
class MLTrainingOrchestrator:
    ...
    async def run_training_pipeline(
        ...
        picks_service_instance = PicksService(
            learning_weights=self.learning_service.get_learning_weights()
        )
        ...
        all_matches = (
            await self.training_data_service.fetch_comprehensive_training_data(
                leagues=leagues,
                days_back=days_back,
        ...
        for match in iterator:
            ...
            if match.home_team.name not in team_stats_cache:
                ...
                ] = self.statistics_service.create_empty_stats_dict()
            ...
            home_stats = self.statistics_service.convert_to_domain_stats(
                match.home_team.name, raw_home
            )
            away_stats = self.statistics_service.convert_to_domain_stats(
            ...
            try:
                ...
                if cached_result and "suggested_picks" in cached_result:
                    ...
                    try:
                        ...
                        suggested_picks_container = MatchSuggestedPicks(
                            match_id=match.id
                        )
                ...
                if not suggested_picks_container:
                    suggested_picks_container = (
                        picks_service_instance.generate_suggested_picks(
                            match=match,
                            home_stats=home_stats,
                            away_stats=away_stats,
                            league_averages=league_averages,
                ...
                for pick in picks_to_process:
                    result_str, payout = self.resolution_service.resolve_pick(
                        pick, match
                    )
                ...
                match_history.append(
                    {
                        ...
                        "predicted_home_goals": round(
                            prediction.predicted_home_goals, 2
                        ),
                        "predicted_away_goals": round(
                            prediction.predicted_away_goals, 2
                        ),
                        "actual_home_goals": match.home_goals,
                        "actual_away_goals": match.away_goals,
                        "was_correct": self._get_predicted_winner(prediction)
                        == self._get_actual_winner(match),
                        "confidence": round(prediction.confidence, 3),
                        "home_win_probability": round(
                            prediction.home_win_probability, 4
                        ),
                        "draw_probability": round(prediction.draw_probability, 4),
                        "away_win_probability": round(
                            prediction.away_win_probability, 4
                        ),
                        "picks": picks_list,
            ...
            self.statistics_service.update_team_stats_dict(
                raw_home, match, is_home=True
            )
            self.statistics_service.update_team_stats_dict(
                raw_away, match, is_home=False
            )
