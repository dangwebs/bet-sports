...
class MongoRepository:
    ...
    def save_match_prediction(self, match_id: str, league_id: str, data: dict, ttl_seconds: int = 86400):
    ...
    def get_match_prediction(self, match_id: str) -> Optional[dict]:
        ...
        if doc and doc.get('expires_at') and doc['expires_at'].replace(tzinfo=None) > datetime.utcnow():
            return doc.get('data')
        return None
    ...
    def bulk_save_predictions(self, predictions_data: List[dict]):
        ...
        for p in predictions_data:
            ...
            operations.append(
                UpdateOne(
                    {"match_id": p['match_id']},
                    {"$set": {"league_id": p['league_id'], "data": p['data'], "expires_at": expires_at, "last_updated": get_current_time()}},
                    upsert=True
                )
            )
    ...
    def get_all_active_predictions(self) -> List[dict]:
        ...
        return [{"match_id": doc['match_id'], "prediction": doc['data'], "last_updated": doc.get('last_updated')} for doc in docs]

    def save_cached_response(self, endpoint: str, data: dict, params: dict = None, ttl_seconds: int = 3600):
